...
async def detalle_item_guest(inventario_id: int):
    ...
    return {
        ...
        "item": {
            ...
            "imagen_url": item.get("foto_ia_url"),
    ...
    for it in items_raw:
        ...
        enriquecidos.append({
            **it,
            "nombre_comun": planta.get("nombre_comun"),
            "nombre_cientifico": planta.get("nombre_cientifico"),
            "imagen_url": inv.get("foto_ia_url"),
...
async def reenviar_cotizacion(
    ...
    # ✅ Obtener sub_cotizaciones primero (sin joins complicados)
    subs_resp = db.table("sub_cotizaciones").select(
        "vivero_id, total_estimado, estado"
    ...
    # ✅ Para CADA vivero, obtener datos y enviar notificación
    for sub in subs:
        vivero_id = sub.get("vivero_id")
        total_vivero = sub.get("total_estimado", 0)
...
async def duplicar_y_reenviar(
    ...
    return {
        ...
        "nombre_proyecto": c.get("prompt_original"),
        "total_estimado": float(c.get("total_estimado") or 0),
        "mensaje": f"Cotización duplicada con ID {nueva_cot_id}.",
    }
...
async def agregar_items_a_cotizacion(
    ...
    return {
        ...
        "mensaje": f"✅ Agregados {len(items_para_agregar)} producto(s)",
    }
